...
(define-map ownership-history
  { listing-id: uint, owner-index: uint }
  {
    owner: principal,
    acquired-at: uint,
    purchase-price: uint
...
(define-map category-trends
  { category: (string-ascii 50), month-year: (string-ascii 7) }
  { purchase-count: uint }
)
...
(define-map ownership-indices { listing-id: uint } { current-index: uint })
...
(define-private (get-current-month-year)
  ...
  (concat 
    (to-ascii (/ block-height u4320)) 
    (concat "-" (to-ascii (/ (mod block-height u4320) u360)))
  )
)
  ...
  (let 
    (
      ...
      (current-data (default-to { purchase-count: u0 } 
                     (map-get? category-trends { category: category, month-year: month-year })))
    )
    (map-set category-trends 
      { category: category, month-year: month-year }
...
(define-private (get-ownership-index (listing-id uint))
  (default-to { current-index: u0 } (map-get? ownership-indices { listing-id: listing-id }))
)

;; Update ownership history for a listing
(define-private (record-ownership (listing-id uint) (new-owner principal) (price uint))
  (let 
    (
    ...
    (map-set ownership-history
      { listing-id: listing-id, owner-index: next-index }
      { owner: new-owner, acquired-at: (get-current-time), purchase-price: price }
    )
...
(define-public (create-listing 
  ...
  (let ((listing-id (get-next-listing-id)))
    ...
    (map-set ownership-history
      { listing-id: listing-id, owner-index: u0 }
      { owner: tx-sender, acquired-at: (get-current-time), purchase-price: u0 }
    )
...
(define-public (purchase-asset (listing-id uint))
  (let
    ...
    (asserts! (is-none (map-get? purchases { listing-id: listing-id, buyer: buyer })) ERR-ALREADY-PURCHASED)
    
    ;; Transfer platform fee to contract owner
    (unwrap! (stx-transfer? platform-fee buyer CONTRACT-OWNER) ERR-PLATFORM-FEE-TRANSFER-FAILED)
...
(define-public (submit-review (listing-id uint) (score uint) (comment (string-utf8 300)))
  (let
    (
      (listing (unwrap! (map-get? listings { listing-id: listing-id }) ERR-LISTING-NOT-FOUND))
...
(define-public (resell-asset (listing-id uint) (new-price uint))
  (let
    (
      (listing (unwrap! (map-get? listings { listing-id: listing-id }) ERR-LISTING-NOT-FOUND))
      (purchase (unwrap! (map-get? purchases { listing-id: listing-id, buyer: tx-sender }) ERR-NOT-PURCHASED))
...
(define-read-only (has-purchased (listing-id uint) (user principal))
  (is-some (map-get? purchases { listing-id: listing-id, buyer: user }))
)
...
(define-read-only (get-category-trend (category (string-ascii 50)) (month-year (string-ascii 7)))
...
(define-read-only (get-ownership-history (listing-id uint) (index uint))
  (map-get? ownership-history { listing-id: listing-id, owner-index: index })
)
